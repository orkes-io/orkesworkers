...
public class OrkesWorkersApplication {
  ...
  public TaskRunnerConfigurer taskRunnerConfigurer(
      List<Worker> workersList, TaskClient taskClient) {
    log.info("Starting workers : {}", workersList);
    TaskRunnerConfigurer runnerConfigurer =
        new TaskRunnerConfigurer.Builder(taskClient, workersList)
  ...
  /**
   * Reads properties from the location specified in <code>ORKES_ACADEMY_CONFIG_FILE</code> and sets
  ...
  public ToolCallbackProvider weatherTools(
      AlwaysFailingTaskWorker worker,
      DHLWorker dhlWorker,
      DynamicTaskArrayPreForkWorker worker2,
      FedExWorker fedExWorker) {
    return MethodToolCallbackProvider.builder()
        .toolObjects(worker, dhlWorker, worker2, fedExWorker)
        .build();
  }
