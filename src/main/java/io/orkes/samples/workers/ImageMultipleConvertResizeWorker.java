...
public class ImageMultipleConvertResizeWorker {
  ...
  @Tool(
      description =
          "Creates dynamic tasks for converting a single image to multiple formats and sizes")
  public Map<String, Object> imageMultipleConvertResize(
      @ToolParam(description = "Input parameters for multiple image conversion options")
          ImageMultipleConversionInput input) {
    ...
    try {
      ...
      for (String outputFormat : input.getOutputFormats()) {
        for (Size size : input.getOutputSizes()) {
          String taskRefName =
              String.format(
                  "%s_%s_%sx%s_%d", dynamicTaskName, outputFormat, size.width, size.height, i++);
      ...
      throw new RuntimeException(
          "Failed to prepare multiple image conversion tasks: " + e.getMessage(), e);
    }
  }
}
