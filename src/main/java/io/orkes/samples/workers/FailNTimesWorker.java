...
public class FailNTimesWorker {
  ...
  @Tool(
      description =
          "A task that fails N times before succeeding, used for testing retry mechanisms")
  public Map<String, Object> failNTimes(
    ...
    if (input.getRetryCount() == 0) {
      result.put(
          "initialStartTime",
          Instant.ofEpochMilli(input.getStartTime()).atZone(ZoneId.of(ZONE)).toString());
      result.put("initialTaskExecutionTime", Instant.now().atZone(ZoneId.of(ZONE)).toString());
      result.put(
          "initialScheduledTime",
          Instant.ofEpochMilli(input.getScheduledTime()).atZone(ZoneId.of(ZONE)).toString());
    }
    ...
    if (failedCount >= input.getTimesToFail()) {
      result.put(
          "outputVal",
          "This task completed failing for configured number of times - " + input.getTimesToFail());
      return result; // Returns with success status
    }

    result.put("failedCount", ++failedCount);
    result.put(
        "outputVal", "This task fails (for testing) for " + input.getTimesToFail() + " times");

    // Throw exception to indicate failure
    throw new RuntimeException(
        "Task failing as per configuration: "
            + failedCount
            + " of "
            + input.getTimesToFail()
            + " failures");
  }
}
