...
public class GenerateThumbnailsWorker {
  ...
  public Map<String, Object> generateThumbnails(
      @ToolParam(description = "Input parameters for thumbnail generation")
    ...
    try {
      ...
      List<ThumbnailInfo> thumbnailInfoList =
          thumbnailGenerate(input.getFileLocation(), outputFileNamePrefix, fileExtension);
  ...
  public List<ThumbnailInfo> thumbnailGenerate(
      String inputFileLocation, String outputFileLocationPrefix, String fileExtension)
      throws Exception {
    ...
    String cmd =
        "ffmpeg -i "
            + inputFileLocation
            + " -vf "
            + " \"select='gt(scene\\,0.5)',metadata=print:file="
            + timeTxtFile
            + "\" "
            + " -vsync vfr "
            + " "
            + outputFileLocationPrefix
            + "%05d"
            + "."
            + fileExtension;
  ...
  public static List<ThumbnailInfo> loadIndos(
      String timeTxtFile, String filenamePrefix, String fileExtension) throws IOException {
  ...
  private Map<String, Object> uploadFiles(List<ThumbnailInfo> thumbnailInfos) {
    ...
    for (ThumbnailInfo thumbnailInfo : thumbnailInfos) {
      String url =
          S3Utils.uploadToS3(thumbnailInfo.getFileLocation(), Regions.US_EAST_1, s3BucketName);
