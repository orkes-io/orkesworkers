...
public class TranscribeWorker {
  ...
  @Tool(
      description =
          "Transcribes audio files using OpenAI's Whisper API and adds subtitles to the video")
  public Map<String, Object> executeTranscribe(
    ...
    try {
      ...
      java.nio.file.Files.copy(
          in, Paths.get(tmpInputFileName), StandardCopyOption.REPLACE_EXISTING);
      ...
      try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
        try (CloseableHttpResponse response = (CloseableHttpResponse) client.execute(post)) {
          ...
          String outputFileWithSubtitles =
              "/tmp/" + UUID.randomUUID().toString() + "-" + outputFileNameWithSubtitles;
          ...
          String subtitleFileUrl =
              S3Utils.uploadToS3(tmpOutputFileName, Regions.US_EAST_1, s3BucketName);
          String videoFileWithSubtitlesUrl =
              S3Utils.uploadToS3(outputFileWithSubtitles, Regions.US_EAST_1, s3BucketName);
  ...
  public void addSubtitleToFile(
      String inputFileLocation, String subtitleLocation, String language, String outputFile)
      throws Exception {
