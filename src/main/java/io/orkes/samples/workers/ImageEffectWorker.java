...
public class ImageEffectWorker {
  ...
  public Map<String, Object> applyImageEffect(
      @ToolParam(description = "Input parameters for applying image effects")
          ImageEffectInput input) {
    ...
    try {
      ...
      String outputFileName =
          "/tmp/" + UUID.randomUUID().toString() + "-" + recipe.name() + "." + fileExtension;

      if (recipe == RECIPE.SEPIA) {
        Integer sepiaIntensityThreshold =
            Doubles.tryParse(recipeParameters.get("sepiaIntensityThreshold").toString()).intValue();
  ...
  private RECIPE validateRecipeNames(String recipeName) throws Exception {
    ...
    if (!supportedRecipes.contains(recipe)) {
      throw new Exception(
          "Recipe: " + recipeName + " not supported. Supported recipes: " + supportedRecipes);
    }
    return recipe;
  }

  private void sepia(String inputImage, Integer sepiaIntensityThreshold, String outputImage)
      throws Exception {
