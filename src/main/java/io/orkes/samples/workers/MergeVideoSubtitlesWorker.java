...
public class MergeVideoSubtitlesWorker {
  ...
  public Map<String, Object> mergeVideoSubtitles(
      @ToolParam(description = "Input parameters for merging videos and subtitles")
          MergeVideoSubtitlesInput input) {
    ...
    try {
      ...
      String originalFileName =
          Files.getNameWithoutExtension(fileLocation) + "." + Files.getFileExtension(fileLocation);
      ...
      java.nio.file.Files.copy(
          originalFileNameStream,
          Paths.get(tmpOriginalFileName),
      ...
      for (Map<String, Object> fileInfo : input.getFiles_to_merge()) {
        ...
        String videoFileName =
            Files.getFileExtension(videoFileUrl)
                + "."
                + Files.getNameWithoutExtension(videoFileUrl);
        String subTitleFileName =
            Files.getFileExtension(subtitleFileUrl)
                + "."
                + Files.getNameWithoutExtension(subtitleFileUrl);
        ...
        java.nio.file.Files.copy(
            videoFileNameStream, Paths.get(tmpVideoFileName), StandardCopyOption.REPLACE_EXISTING);
        tmpFiles.add(tmpVideoFileName);
        inputFilesListContent =
            inputFilesListContent + "file " + tmpVideoFileName + System.lineSeparator();
        ...
        java.nio.file.Files.copy(
            subtitleFileNameStream,
            Paths.get(subtitleFileName),
            StandardCopyOption.REPLACE_EXISTING);
        tmpFiles.add(subtitleFileName);
        subtitllesFileListContent =
            subtitllesFileListContent + subtitleFileName + System.lineSeparator();
      }
      ...
      FileUtils.writeStringToFile(
          new File(inputFilesListName), inputFilesListContent, Charset.forName("UTF-8"));
      FileUtils.writeStringToFile(
          new File(subtitlesFileListName), subtitllesFileListContent, Charset.forName("UTF-8"));
      ...
      String outputMergedSubtitleFile =
          "/tmp/" + UUID.randomUUID().toString() + "." + outputSubtitleFileFormat;
      ...
      String subTitleUrl =
          S3Utils.uploadToS3(outputMergedSubtitleFile, Regions.US_EAST_1, s3BucketName);
  ...
  public void addMergedSubToVideo(
      String originalFileName, String outputMergedSubtitleFile, String outputFileName)
      throws Exception {
  ...
  public void extractMergedSub(String mergedFileName, String outputMergedSubtitleFile)
      throws Exception {
    // ffmpeg -i output.m4v -map 0:s:0 output.srt
    String cmd = "ffmpeg -i " + mergedFileName + "  -map 0:s:0  " + outputMergedSubtitleFile;
  ...
  public void mergeVideos(
      String inputFilesListName, String subtitlesFileListName, String outputFileName)
      throws Exception {
    // ffmpeg -f concat -safe 0 -i input.txt -f srt -i subtitles.txt -c copy -c:s mov_text
    // output.m4v
    String cmd =
