...
public class SplitVideoWorker {
  ...
  public static class SplitVideoInput {
    ...
    private Object
        durationInSeconds; // Using Object to handle potential parsing issues as in original code
  }
  ...
  public Map<String, Object> executeSplitVideo(
    ...
    try {
      ...
      Integer durationInSeconds =
          Doubles.tryParse(input.getDurationInSeconds().toString()).intValue();

      Path tmpOutputdir =
          java.nio.file.Files.createTempDirectory(Paths.get("/tmp"), "split-video-");
  ...
  private List<String> uploadFiles(Path path, String fileNamePrefix) {
    List<File> files =
        Lists.newArrayList(
            path.toFile()
                .listFiles((FileFilter) new PrefixFileFilter(fileNamePrefix, IOCase.SENSITIVE)));
    String s3BucketName = "image-processing-orkes";
    List<String> urls = Lists.newArrayList();
    for (File file : files.stream().sorted().collect(Collectors.toList())) {
  ...
  public void splitVideo(
    ...
    // ffmpeg -i netflix.mp4 -c copy -map 0 -segment_time 30 -f segment -reset_timestamps 1
    // output%03d.mp4
    String cmd =
        "ffmpeg -i "
            + inputFileLocation
